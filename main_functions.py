...
def geteloratings(elos, best_player_so_far, improved, total_improved):
    ...
    if improved == 1 and total_improved % config.checkpoint_frequency == config.checkpoint_frequency - 1:
        print('')
        print('---CHECKPOINT: winrate against mcts with', budget_mcts, 'sims (', config.CPUS * loop_number_mcts,
              'games)')
        time.sleep(0.01)
...
def printstates(player):
    ...
    for i in range(len(part_states)):
        ...
        if int(turn) <= 4:
            if Qchilds[3] > Qchilds[0] or Qchilds[3] > Qchilds[1] or Qchilds[3] > Qchilds[2] or Qchilds[3] > Qchilds[
                4] or Qchilds[3] > Qchilds[5] or Qchilds[3] > Qchilds[6]:
                getbreak = 0
